#!/usr/bin/env node
// FROZEN v1-legal (v12 tag) applied to an arbitrary genre: originals + v1 substrate ->
...
for (const f of readdirSync(SUB).filter((x) => x.endsWith('.txt'))) {
  ...
  const eligible = (tag) => {
    ...
    if (row.cls === 'PERSON') {
      ...
      for (const m of orig.matchAll(re)) if (OFFICIAL_TITLE_RE.test(orig.slice(Math.max(0, m.index - 30), m.index))) return true
    }
    return false
  }
