...
for (const f of readdirSync(SUB).filter((x) => x.endsWith('.txt'))) {
  ...
  const eligible = (tag) => {
    ...
    if (row.cls === 'COMPANY') { const defRe = new RegExp('[(“"‘\x27]\\s*(?:the\\s+)?' + esc(row.span.replace(/^the\s+/i,'')) + '\\s*[”"’\x27)]', 'i'); if (defRe.test(orig)) return false; const ptoks = new Set(tbl.filter(x=>x.cls==='PERSON').flatMap(x=>x.span.split(/\s+/).map(w=>w.toLowerCase()))); return !folded.toLowerCase().split(/\s+/).some(w=>w.length>=3&&ptoks.has(w)) }
    if (row.cls === 'PERSON') {
  ...
  for (const rules of [R.L1_RULES_NO_CODE, R.L1C_RULES, R.L3_RULES, R.L4_RULES, R.L6_RULES, R.L8_RULES, R.L9_RULES]) for (const { re } of rules) add(re)
