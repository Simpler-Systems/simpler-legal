...
function utf16NoBom(buf) {
    ...
    const pairs = n / 2;
    if (odd > pairs * 0.3 && even < pairs * 0.05)
        return 'utf-16le';
    if (even > pairs * 0.3 && odd < pairs * 0.05)
        return 'utf-16be';
    return null;
}
...
function hasMsgStreams(buf) {
    outer: for (let i = 0; i + MSG_STREAM.length <= buf.length; i++) {
        for (let k = 0; k < MSG_STREAM.length; k++)
            if (buf[i + k] !== MSG_STREAM[k])
                continue outer;
        return true;
    }
    return false;
}
const FILE = { what: 'This file', unit: 'line' };
...
export function mailHeaderLine(text) {
    const lines = text.split(EOL);
    let start = -1, count = 0, name = '', value = '';
    let transport = new Set(), origin = false, encodedPart = false;
    const close = () => {
        const v = value.trim();
        if (name && TRANSPORT[name] && TRANSPORT[name].test(v)) {
            transport.add(name);
            if (name === 'content-transfer-encoding' && /^(?:base64|quoted-printable)\b/i.test(v))
                encodedPart = true;
        }
    ...
    for (let i = 0; i <= lines.length; i++) {
        ...
        if (start >= 0 && /^[ \t]+\S/.test(bare)) {
            value += ' ' + bare.trim();
            continue;
        } // folded
        if (start >= 0) {
            close();
            if (count >= 2 && (encodedPart || (transport.size >= 2 && origin)))
                return start + 1;
            start = -1;
        }
    }
...
function readsAsText(bytes) {
    ...
    try {
        ...
        if (b.filter((x) => x >= 0x80).length > b.length * 0.2)
            return false;
    ...
    if (n >= 24)
        return true;
    if ((s.match(/[A-Za-zÀ-ɏ]{3,}/g) || []).length >= 2 || /[^\P{L}\u0000-ɏ]{2,}/u.test(s))
        return true;
...
function hexBytes(token) {
    const out = new Uint8Array(token.length >> 1);
    for (let i = 0; i < out.length; i++)
        out[i] = parseInt(token.substr(i * 2, 2), 16);
    return out;
}
...
function b32Bytes(token) {
    if ([1, 3, 6].includes(token.length % 8))
        return null;
    const out = [];
    let acc = 0, bits = 0;
...
const cutPieces = (words, keep, min = 1) => words.flatMap((w) => [...w].map((_, i) => (keep === 'head' ? w.slice(0, i + 1) : w.slice(i))).filter((p) => p.length >= min)).map(reEsc).join('|');
...
function safeLinksPiece(core, min = 8) {
    if (core.length > 216)
        return false;
    for (let d = 0; d < 4; d++) {
...
const charClass = (c) => (c >= 65 && c <= 90 ? 1 : c >= 97 && c <= 122 ? 2 : c >= 48 && c <= 57 ? 3 : 4);
...
function encoderShaped(s) {
    const upper = (s.match(/[A-Z]/g) || []).length, lower = (s.match(/[a-z]/g) || []).length;
    const share = upper / Math.max(1, upper + lower);
    if (!/[0-9+/_-]/.test(s) || share < 0.2 || share > 0.8)
        return false;
    let changes = 0;
    for (let i = 1; i < s.length; i++)
        if (charClass(s.charCodeAt(i)) !== charClass(s.charCodeAt(i - 1)))
            changes++;
...
function tokenVerdict(tok, inner = false, query = false) {
    const core = tok.replace(/=+$/, '');
    if (HEX.test(core)) {
        const hex = core.replace(/^0x/, '');
        if (hex.length >= HEX_BLOB)
            return 'blob';
        ...
        if (!/[A-Fa-f]/.test(hex) || !/[0-9]/.test(hex) || /^(?:AD|AE|AF|BA|BB|BD|BE|BF|CA|CD|CF|DE|EC|EE)\d{12,}$/.test(hex))
            return null;
    ...
    if (safeLinksPiece(core))
        return null;
    const long = core.length > DIGEST_MAX && !BASE58.test(core) && encoderShaped(core);
    if (long && !(query && core.length <= LINK_VALUE_MAX))
        return 'blob';
    const bytes = b64Bytes(core);
    if (!bytes)
        return long ? 'blob' : null;
    if (long && ((bytes[0] === 0x1f && bytes[1] === 0x8b) || (bytes[0] === 0x78 && ((bytes[0] << 8) | bytes[1]) % 31 === 0)))
        return 'blob';
    ...
    if (!inner) {
        ...
        if (once.length >= 16 && padded && /^[A-Za-z0-9+/_-]+={1,2}$/.test(once))
            return tokenVerdict(once, true);
        ...
        if (piece) {
            const h = once.replace(/-/g, '');
            return readsAsText(hexBytes(h)) || readsAsText(hexBytes(h.slice(1))) ? 'text' : null;
        }
        if (hexId) {
            const h = hexId[1].replace(/-/g, '');
            return readsAsText(hexBytes(h)) || (h.length % 2 === 1 && readsAsText(hexBytes(h.slice(1)))) ? 'text' : null;
        }
    }
...
const deeperHeads = (tok) => { const out = []; for (let k = 1; k <= 4; k++) {
    const h = tok.match(new RegExp(`^(?:${DEEP_WORD}[_-]){${k}}`));
    if (!h || h[0].length > 48)
        break;
    out.push(h[0].length);
} return out; };
// A word, a number or an initial-capped name, with "-", "_" and "/" as joints: not an encoding.
const wordShaped = (s) => s.split(/[-_/]/).every((w) => /^(?:[A-Z]?[a-z]*|[A-Z]*|[0-9]*)$/.test(w));
...
function blockLine(line) {
    ...
    if (!/^[A-Za-z0-9+/_-]+={0,6}$/.test(s) || !/[A-Za-z0-9]/.test(s) || s.startsWith('--') || /^0x[0-9A-Fa-f]+$/.test(s))
        return null;
...
function wrappedBlock(block) {
    ...
    if (run.length >= 8 && run.every((l) => l[0] === run[0][0]))
        return false;
    ...
    if (run.slice(0, -1).every((l) => l.length === width) && last.length <= width) {
        ...
        if (last.length < width || last.endsWith('='))
            return true;
...
function base64Hold(text) {
    ...
    if (other)
        return { line: lineAt(text, other.index), link: false };
    ...
    for (let i = 0; i < lines.length;) {
        ...
        if (wrappedBlock(run)) {
            ...
            for (let u = k - 1; u >= 0 && u >= k - 64 && /\S$/.test(src[u]) && (!encoderRows || Math.abs(src[k - 1].length - src[k].length) <= 1); u--) {
                if (/:\/\/|^www\.|[?&][\w.%-]*=/.test(src[u].split(/\s/).pop())) {
                    link = true;
                    break;
                }
                if (/\s/.test(src[u].trim()))
                    break;
            }
    ...
    for (let r = 0; r + 1 < raw.length; r++) {
        ...
        if (!cut || !/%[0-9A-Fa-f]{2}|:\/\/|[?=]/.test(cut[1]))
            continue;
        const need = 2 - cut[2].length;
        if (!new RegExp(`^[0-9A-Fa-f]{${need}}`).test(raw[r + 1]))
            continue;
    ...
    for (let r = 0; r < rows.length; r++) {
        ...
        const carried = linkCarry && !/\s/.test(row);
        const inLinkWord = /:\/\/|^www\.|[?&][\w.%-]*=/.test(last) || carried;
        const pathCarried = carried && linkPath;
        const queryCarried = linkCarry && linkQuery && /^\S/.test(row);
        ...
        for (const m of row.matchAll(tokens)) {
            ...
            let tok = m[0];
            if (inV2 && m.index === 0 && owe)
                tok = tok.replace(new RegExp(`^[0-9A-F]{${owe}}`), '');
            if (inV2 && m.index + m[0].length === row.length)
                tok = tok.replace(/-[0-9A-F]?$/, '');
            ...
            if (!(glued && pre[0].length % 4 === 0) && (v2 ? v2.some((p) => tokenVerdict(p, false, query)) : tokenVerdict(m[0], false, query)))
                return { line: r + 1, link: query || carried || /:\/\/|^www\.|[?&][\w.%-]*=/.test(before) };
            if (glued && tokenVerdict(rest, false, query))
                return { line: r + 1, link: query || carried || /:\/\/|^www\.|[?&][\w.%-]*=/.test(before) };
            if (!inV2 && !/(?:-[0-9A-F]{2}.*){2}/.test(m[0]))
                for (const h of deeperHeads(m[0])) {
                    const x = m[0].slice(h);
                    if (x.length >= 16 && !wordShaped(x.replace(/=+$/, '')) && tokenVerdict(x, false, query))
                        return { line: r + 1, link: query || carried || /:\/\/|^www\.|[?&][\w.%-]*=/.test(before) };
                }
            ...
            if ((inner || route || !/\?[^#]*$|#/.test(before)) && m[0].includes('/') && (/:\/\/|^www\./.test(before + m[0]) || inner || pathCarried) && pathSegmentHold(m[0]))
                return { line: r + 1, link: true };
        }
        ...
        if (!tail || !rows[r + 1] || (r <= joinedTo && tail.index === 0))
            continue;
        const pieces = [tail[0]];
        for (let k = r + 1; k < rows.length; k++) {
            ...
            if (!next || wordShaped(next[0].replace(/=+$/, '')))
                break;
            pieces.push(next[0]);
            if (next[0].length < rows[k].length || next[0].endsWith('='))
                break;
        }
        ...
        const slRow = (x, i) => { const v = x.replace(/=+$/, ''); return (i > 0 && i === pieces.length - 1 && v.length <= 5) || safeLinksPiece(v, 4); };
        if (pieces.length > 1 && !wordShaped(joined.replace(/=+$/, '')) && tokenVerdict(joined, false, true) && !pieces.every(slRow))
            return { line: r + 1, link: true };
        // a path printed across rows: each of its segments from its own start, as on one row
        if (pieces.length > 1 && linkPath && pathSegmentHold(joined))
            return { line: r + 1, link: true };
        ...
        const opening = (x, i) => { const b = i === 0 && b64Bytes(x); return !!b && b.length > 0 && 'Mailflow|{'.startsWith(String.fromCharCode(...b)); };
        if (pieces.length > 1 && pieces.some((x) => x.length >= 16 && safeLinksPiece(x.replace(/=+$/, ''), 4)) && !safeLinksPiece(joined.replace(/=+$/, '')) && !pieces.every((x, i) => opening(x, i) || slRow(x, i)))
            return { line: r + 1, link: true };
    }
    return null;
}
...
function unescapePct(s) {
    return s.replace(/(?:%[0-9A-Fa-f]{2})+/g, (run) => {
        try {
            return decodeURIComponent(run);
        }
        catch {
            return run.replace(/%([0-9A-Fa-f]{2})/g, (e, h) => (parseInt(h, 16) < 0x80 ? String.fromCharCode(parseInt(h, 16)) : e));
        }
    });
}
...
function gatewayInner(link, printed = false) {
    ...
    const upTo = (u, next) => { const eq = u.indexOf('='); return eq < 0 ? u : u.slice(0, eq).replace(new RegExp(`${next}$`), ''); };
    const v = (key, next) => { const u = (link.match(new RegExp(`[?&]${key}=([^&\\s]*)`)) || [])[1]; return u && upTo(u, next); };
    ...
    const own = (re) => new RegExp(String.raw `^https?:\/\/(?:[\w-]+\.)*` + re.source, 'i').test(link);
    if (own(/safelinks\.protection\.outlook\.com\//)) {
        const u = v('url', 'data');
        return u ? unescapePct(printed ? u.replace(/%7C[\s\S]*$/i, '') : u) : null;
    }
    if (own(/linkprotect\.cudasvc\.com\//)) {
        const u = v('a', 'c');
        return u ? unescapePct(u) : null;
    }
    if (own(/urldefense(?:\.proofpoint)?\.com\/v1\//)) {
        const u = v('u', 'k');
        return u ? unescapePct(u) : null;
    }
    if (own(/urldefense(?:\.proofpoint)?\.com\/v2\//)) {
        const u = v('u', 'd');
        return u ? unescapePct(u.replace(/-/g, '%').replace(/_/g, '/')) : null;
    }
    ...
    if (!v3) {
        const u = link.match(/[?&][\w.-]+=(https?%3A%2F%2F[^&\s]*)/i);
        return u ? unescapePct(upTo(u[1], '')) : null;
    }
    ...
    let moved = '';
    try {
        moved = new TextDecoder('utf-8', { fatal: true }).decode(Uint8Array.from(atob((v3[2] || '').replace(/-/g, '+').replace(/_/g, '/') + '='.repeat((4 - ((v3[2] || '').length % 4)) % 4)), (c) => c.charCodeAt(0)));
    }
    catch { /* left as "*" */ }
    ...
    return unescapePct(v3[1]).replace(/\*(\*.)?/g, (t) => {
        ...
        if (!moved || n < 1 || at + n > moved.length)
            return t;
        at += n;
...
function gatewayHold(text, depth = 0) {
    ...
    for (let r = 0; r < rows.length; r++) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            if (i === 0 && w.index === 0 && r > 0 && /\S$/.test(rows[r - 1]) && r <= joinedThrough)
                continue;
            let word = w[0], cut = 0;
            ...
            if (w.index + word.length === rows[r].length) {
                for (let k = r + 1; k < rows.length && word.length < GATEWAY_MAX; k++) {
                    ...
                    word += next[0];
                    if (next[0].length < rows[k].length) {
                        cut = word.length - next[0].length;
                        break;
                    }
                }
            }
            ...
            for (const whole of prose ? [word, word.slice(0, cut)] : [word]) {
                for (const g of whole.matchAll(GATEWAY)) {
                    ...
                    if (inner && (base64Hold(inner) || percentHold(inner) || (depth < 2 && gatewayHold(inner, depth + 1))))
                        return starts.filter(([s]) => s <= g.index).pop()[1] + 1;
                }
            }
        }
    }
...
function percentGrid(text, start, end) {
    if (/[\r\n]/.test(text.slice(start, end)))
        return false;
    const num = (i) => /\d/.test(text[i] ?? '');
    let s = start, e = end;
    while (s > 0 && (num(s - 1) || (/[.,]/.test(text[s - 1]) && num(s - 2))))
        s--;
    while (e < text.length && (num(e) || text[e] === '%' || (/[.,]/.test(text[e]) && num(e + 1))))
        e++;
    return s < start && /^(?:\d+(?:[.,]\d+)*%){2,}$/.test(text.slice(s, e));
}
function percentHold(text) {
    for (const m of text.matchAll(/(?:%[0-9A-Fa-f]{2})+(?:\r?\n(?:%[0-9A-Fa-f]{2})+)*/g)) {
        const run = m[0].replace(/\s/g, '');
        if (percentGrid(text, m.index, m.index + m[0].length))
            continue;
        const alnum = run.match(/%(?:3[0-9]|4[1-9A-Fa-f]|5[0-9Aa]|6[1-9A-Fa-f]|7[0-9Aa])/g);
        if (alnum && alnum.length >= 2)
            return lineAt(text, m.index);
        if (inWrappedLink(text, m.index))
            continue;
        let bytes = run.slice(1).split('%').map((h) => parseInt(h, 16));
        while (bytes.length && bytes[0] >= 0x80 && bytes[0] < 0xc0)
            bytes = bytes.slice(1);
        for (let k = bytes.length - 1; k >= 0 && k >= bytes.length - 4; k--) {
            if (bytes[k] < 0x80)
                break;
            if (bytes[k] >= 0xc0) {
                if (bytes.length - k < (bytes[k] >= 0xf0 ? 4 : bytes[k] >= 0xe0 ? 3 : 2))
                    bytes = bytes.slice(0, k);
                break;
            }
        }
        let letters;
        try {
            letters = new TextDecoder('utf-8', { fatal: true }).decode(new Uint8Array(bytes));
        }
        catch {
            continue;
        }
        if ((letters.match(/\p{L}/gu) ?? []).filter((c) => !/\p{Script=Latin}/u.test(c)).length >= 2)
            return lineAt(text, m.index);
    }
    return 0;
}
...
function inWrappedLink(text, index) {
    if (inLink(text, index))
        return true;
    let b = index, e = index;
    while (b > 0 && !/\s/.test(text[b - 1]))
        b--;
    while (e < text.length && !/\s/.test(text[e]))
        e++;
...
function pathBefore(text, index) {
    for (let i = index; i > 0;) {
        const c = text[i - 1];
        if (c === '/')
            return true;
        if (c === '\n' || c === '\r') {
            ...
            if (text[x] === '\n' && text[x - 1] === '\r')
                x--;
            if (!/(?:\/|%[0-9A-Fa-f]{0,2})$/.test(text.slice(Math.max(0, x - 3), x)))
                return false;
            i = x;
            continue;
        }
        ...
        if (c === ',' && !(/^(?:_|%20)/.test(text.slice(i, i + 3)) && /\/[^\s/;.]*%[0-9A-Fa-f]{2}[^\s/;.]*$/.test(text.slice(Math.max(0, i - 1 - 2048), i - 1))))
            return false;
        i--;
    }
    return false;
...
function qpLetterLine(text) {
    for (const m of text.matchAll(/([A-Za-z]?)((?:=[0-9A-F]{2})+)([A-Za-z]?)/g)) {
        ...
        if (/\p{Ll}\p{Lu}/u.test(before + ch + after))
            continue;
        if (inLink(text, m.index))
            continue;
        ...
        if (bytes.length === 1 && after === 'h' && !/\p{L}/u.test(text[end] || ''))
            continue;
        ...
        if (bytes.length === 1 && kv && /[A-Z0-9+/]/.test(kv[1]))
            continue;
        return lineAt(text, m.index);
    }
    return 0;
...
function qpSoftBreakLine(text) {
    const lines = text.split(EOL);
    for (let i = 0; i + 1 < lines.length; i++) {
        const l = lines[i];
        if (l.length < 60 || l.length > 76 || !/[^\s=]=$/.test(l) || !/\S[ \t]+\S/.test(l) || !/^\S/.test(lines[i + 1]))
            continue;
        // A link wrapped after its query key ("…/view?id=" / "123456789").
        const last = l.slice(l.search(/\S+$/));
        if (/:\/\/|www\./i.test(last) || /[?&][A-Za-z_][\w.-]*=$/.test(last))
            continue;
        ...
        if (/=(?![0-9A-Fa-f]{2})./.test(l.slice(0, -1)))
            continue;
        ...
        if (run && run.length % 4 === 0 && !wordShaped(run.slice(0, -1)) && !tokenVerdict(run))
            continue;
        return i + 1;
    }
    return 0;
...
export const MAIL_LIMIT = 'an email can carry base64-encoded parts this app can’t read, so it can’t certify the export. Open the message, save the body as text or PDF, and drop that and each attachment on its own.';
/** The first reason this decoded text must not reach the engine, or null. */
export function contentHold(text, where = FILE) {
    const { what, unit } = where;
    const at = (n) => (where.at ? where.at(n) : `${unit} ${n}`);
    ...
    const linkReason = (line) => `${what} contains encoded content inside a link (from ${at(line)}): part of the link’s address is written in an encoding this app cannot read, so it cannot mask a name or an email address it carries. Delete the link, or replace it with the words it shows, and drop the file again.`;
    const b64 = base64Hold(text);
    if (b64)
        return b64.link ? { format: 'encoded', line: b64.line, reason: linkReason(b64.line), kind: 'link' } : { format: 'encoded', line: b64.line, reason: `${what} contains encoded content (from ${at(b64.line)}), such as an email attachment, that this app cannot read, so it cannot mask what is inside. Save the attachment as its own file and drop that; if the encoded block is not needed, delete it and drop the file again.` };
    const gate = gatewayHold(text);
    if (gate)
        return { format: 'encoded', line: gate, reason: linkReason(gate), kind: 'link' };
    // Its own reason: nothing is attached, and "save the attachment" sent the lawyer looking for one.
    const pct = percentHold(text);
    if (pct)
        return { format: 'encoded', line: pct, reason: `${what} contains percent-encoded content (from ${at(pct)}): letters written as “%” and two hex digits, which this app cannot read, so it cannot mask a name they spell. Replace them with the letters they stand for, or delete them, and drop the file again.`, kind: 'percent' };
    ...
    if (soft)
        return { format: 'encoded', line: soft, reason: `${what} contains quoted-printable email encoding (${at(soft)}), which breaks words across lines with “=”, so a name can escape the mask. Open the message in your mail program, save it as text, and drop that.`, kind: 'qp' };
    const qp = qpLetterLine(text);
    if (qp)
        return { format: 'encoded', line: qp, reason: `${what} contains quoted-printable email encoding (${at(qp)}) — a letter written as “=E9” splits the word it is in, so a name can escape the mask. Open the message in your mail program, save it as text, and drop that.`, kind: 'qp' };
    return null;
}
...
export const DOCX_SAVED_CHANNELS = {
...
export function docxContentHold(items) {
    const parts = new Map();
    for (const i of items) {
        ...
        if (!(i.kind in DOCX_SAVED_CHANNELS) || i.attr || (i.hidden && !inMainFlow(i)) || i.rev === 'del')
            continue;
        if (!parts.has(i.part))
            parts.set(i.part, []);
    ...
    for (const list of order) {
        ...
        const hiddenOnly = (n) => {
            ...
            const by = new Map();
            for (const i of own)
                by.set(num(i), (by.get(num(i)) ?? true) && !!i.hidden);
...
export function detectFormat(buf, filename = '') {
    ...
    if (buf.length >= 4 && buf[0] === 0x50 && buf[1] === 0x4b && ((buf[2] === 3 && buf[3] === 4) || (buf[2] === 5 && buf[3] === 6) || (buf[2] === 7 && buf[3] === 8))) {
        if (ext === 'docx' || ext === '')
            return { format: 'docx', route: 'docx-walker' };
        if (ext === 'xlsx')
            return { format: 'xlsx', route: 'refuse', reason: 'Spreadsheets aren’t supported yet — save each sheet as CSV (structure and formulas are lost).' };
        if (ext === 'pptx')
            return { format: 'pptx', route: 'refuse', reason: 'Slide decks aren’t supported yet — File → Export → outline text (images and charts don’t survive).' };
    ...
    if (buf.length >= 4 && buf[0] === 0xd0 && buf[1] === 0xcf && buf[2] === 0x11 && buf[3] === 0xe0) {
        if (ext === 'msg' || hasMsgStreams(buf))
            return { format: 'msg', route: 'refuse', reason: 'Outlook .msg isn’t supported yet — in Outlook: File → Save As → Text, and save each attachment as its own file.' };
    ...
    if (['eml', 'mbox', 'mbx', 'emlx'].includes(ext))
        return { format: ext, route: 'refuse', reason: `Email files aren’t supported yet — ${MAIL_LIMIT}` };
    if (['mht', 'mhtml'].includes(ext))
        return { format: ext, route: 'refuse', reason: 'Web archives (.mht) aren’t supported yet — they carry their pictures and attachments base64-encoded, which this app can’t read. Open it in a browser or Word, save it as text or .docx, and drop that.' };
    ...
    if (hold)
        return { format: hold.format, route: 'refuse', reason: hold.reason };
