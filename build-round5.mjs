#!/usr/bin/env node
// ROUND 5 — fresh paper for the FS7+P0g fixes (walk-forward law: fixes prove on documents
// they have never seen). 40 docs, arrival order, zero overlap with oos/round4:
//   15 edgar-r5   fresh queries: "change in control agreement", "retention agreement"
//   10 us-r5      CAP F.3d unused cases (ascending volumes — round4 drew descending)
//   10 sg-r5      eLitigation 2022 SGHC (fresh year)
//    5 sghcf-r5   2023 SGHCF letter-coded (pseudonym-preservation)
import { readFileSync, writeFileSync, mkdirSync, readdirSync, statSync, existsSync } from 'node:fs'
import { join } from 'node:path'
import { fileURLToPath } from 'node:url'

const ROOT = fileURLToPath(new URL('.', import.meta.url))
const OUT = join(ROOT, 'raw', 'round5')
...
async function collectEdgar() {
  const used = new Set()
  for (const m of ['oos-manifest.json', 'round4-manifest.json']) {
    for (const d of JSON.parse(readFileSync(join(ROOT, 'pii-bench', m), 'utf8')).docs) if (d.cite) used.add(d.cite)
  }
  const perCik = new Map()
  let n = 0
  for (const q of ['%22change in control agreement%22', '%22retention agreement%22']) {
    for (let from = 0; from < 120 && n < 15; from += 10) {
      ...
      for (const h of hits) {
        ...
        perCik.set(cik, 1)
        n++
        const name = `edgar-r5-${String(n).padStart(2, '0')}.txt`
        writeFileSync(join(OUT, name), text)
        manifest.push({ file: name, source: 'edgar-r5', title: h._source.display_names?.[0] || cik, cite: acc, url: `https://www.sec.gov/Archives/edgar/data/${cik}/${acc.replace(/-/g, '')}/${file}`, words: w })
...
function collectUS() {
  ...
  outer: for (const vol of vols) {
    for (const file of walkJson(join(ROOT, 'raw', vol))) {
      ...
      n++
      const name = `us-r5-${String(n).padStart(2, '0')}.txt`
      writeFileSync(join(OUT, name), `Title: ${j.name_abbreviation}\nSource: static.case.law (${vol}, CAP id ${j.id})\n\n${text}`)
      manifest.push({ file: name, source: 'us-r5', title: j.name_abbreviation, cite: j.citations?.[0]?.cite || '', url: `https://static.case.law/f3d/${vol.slice(4)}.zip (CAP id ${j.id})`, words: w })
...
async function collectSG(prefix, year, court, want, startN = 1) {
  ...
  for (let i = startN; i <= 300 && n < want && misses < 40; i++) {
    ...
    try {
      ...
      if (prefix === 'sghcf-r5' && !text.match(/\b([A-Z]{1,4})\s+v\s+([A-Z]{1,4})\b/)) continue
      n++
...
await collectEdgar()
collectUS()
await collectSG('sg-r5', 2022, 'SGHC', 10)
await collectSG('sghcf-r5', 2023, 'SGHCF', 5)
writeFileSync(join(ROOT, 'pii-bench', 'round5-manifest.json'), JSON.stringify({ builtAt: '2026-07-23', protocol: 'ROUND 5: FS7+P0g fixes first-contact; blind swarm gold BEFORE engine; published as drawn; arrival order', docs: manifest }, null, 1))
console.log(`\nround 5: ${manifest.length} docs -> raw/round5/`)
